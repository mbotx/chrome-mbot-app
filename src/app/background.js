...
function setupSerial(port) {
    ...
    port.onMessage.addListener(function(msg) {
        if (msg.method == "list") {
            ...
            chrome.serial.connect(msg.path, {
                    bitrate: 115200
    ...
    chrome.serial.onReceive.addListener(function(obj) {
        ...
        if (len > 0) {
            scratchPort.postMessage({
                    buffer: buffer
                });
            port.postMessage({
                    event: "__DATA_RECEIVED__",
                    data: buffer
                });
        }
    });
...
function setupBluetooth(port) {
    ...
    chrome.bluetooth.onDeviceAdded.addListener(function(device) {
        port.postMessage({
                event: "__DEVICE_ADDED__",
                device: device
            });
    });
    chrome.bluetooth.onDeviceRemoved.addListener(function(device) {
        port.postMessage({
                event: "__DEVICE_REMOVED__",
                device: device
            });
    });
    chrome.bluetooth.onDeviceChanged.addListener(function(device) {
        port.postMessage({
                event: "__DEVICE_CHANGED__",
                device: device
            });
    });
